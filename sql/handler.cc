...
static my_xid wsrep_order_and_check_continuity(XID *list, int len)
{
  ...
  for (int i= 0; i < len; ++i)
  {
    ...
    else if (i > 0 && wsrep_xid_seqno(list + i) != cur_seqno + 1)
    {
      /* More than one wsrep XID found in the recovery list, but the
...
#ifdef WITH_WSREP
/*
   Record the wsrep xid for a given xid.

   @param hash_arg The hash to search for the xid.
   @param xid_arg The xid to record the wsrep xid for.
   @param ptr_mem_root The memory root to allocate the wsrep xid for.
   @param wsrep_xid_arg The wsrep xid to record.

   @return true if the xid_recovery_member was not found, or if memory allocation
           for the wsrep xid failed.
           false otherwise.
 */
static bool xid_member_record_wsrep_xid(HASH *hash_arg, my_xid xid_arg,
                                        MEM_ROOT *ptr_mem_root,
                                        XID *wsrep_xid_arg)
{
  xid_recovery_member *member= (xid_recovery_member *) my_hash_search(
      hash_arg, (uchar *) &xid_arg, sizeof(xid_arg));
  if (!member)
    return true;
  member->wsrep_xid= (XID *) alloc_root(ptr_mem_root, sizeof(XID));
  if (!member->wsrep_xid)
    return true;
  *member->wsrep_xid= *wsrep_xid_arg;
  return false;
}
#endif /* WITH_WSREP */
...
#ifdef WITH_WSREP
          /* Note: This is a patch for backwards compatibility only.
             Wsrep XIDs are not used for 2PC anymore since version 12.1.
             The wsrep specific behavior can be removed once there is no
             need to support recovery from pre-12.1 data directory.
          */
          if (wsrep_is_wsrep_xid(&info->list[i]) &&
              xid_member_record_wsrep_xid(info->commit_list, x, info->mem_root,
                                          &info->list[i]))
          {
            info->error= true;
            sql_print_error(
                "Error recording wsrep xid at xarecover_handlerton");
            break;
          }
#endif /* WITH_WSREP */
        }
...
#ifdef WITH_WSREP
  /*
    Unset dry run if wsrep is running without binlog and the
    node is recovering from SST. This is needed for recovery to work in
    the case a joiner that does not have binlog on receives SST from donor
    that has binlog on.
  */
  if (wsrep_emulate_bin_log && wsrep_is_recovering_from_sst())
  {
    info.dry_run= false;
  }
#endif /* WITH_WSREP */
