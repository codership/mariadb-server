    ...
    while ((got= hton->recover(info->list, info->len)) > 0 )
    {
      ...
      */
      ...
      for (int i=0; i < got; i ++)
      {
...
#ifdef WITH_WSREP
        bool wsrep_must_commit_xid= false;
        if (!is_server_xid && wsrep_is_wsrep_xid(&info->list[i]))
        {
          x= wsrep_xid_seqno(&info->list[i]);
          wsrep_must_commit_xid= wsrep_emulate_bin_log && x <= wsrep_limit;
        }
#endif
        ...
        if (IF_WSREP(!wsrep_must_commit_xid && info->dry_run,
                     info->dry_run))
        {
          info->found_my_xids++;
          continue;
        ...
        */
        ...
        if (IF_WSREP(wsrep_must_commit_xid, false) ||
